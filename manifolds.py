'''
File: manifolds.py
Created Date: Tuesday February 1st 2022 - 04:02pm
Author: Ammar Mian
...
-----
Implementation of pymanopt compatible manifolds
-----
...
def dist_texture(tau_0,tau_1):

   dist = np.sqrt(np.linalg.norm(np.log(np.reciprocal(tau_0)*tau_1))**2)
   return dist
...
class ScaledGaussianFIM(WeightedProduct):
    """Manifold Product of SHPD_d and StriclyPositiveVectors representing
    the scaled-Gaussian model with Fisher Information metric.
    Attributes
    ----------
    _d : int
        Size of covariance matrix.
    _n :  int
        Size of texture parameter
    _k : int, optional
        Number of tuples of parameters to represent. Default is 1.
    """

    def __init__(self, d, n, k=1):
        self._d = d
        self._n = n
        self._k = k
        weights = (1/d, 1/n)
        manifolds = (SpecialHermitianPositiveDefinite(d, k),
...
class KroneckerHermitianPositiveScaledGaussian(WeightedProduct):
    """Manifold Product of SHPD_a, SHPD_b and StriclyPositiveVectors representing
    the Kronecker structured scaled-Gaussian model with Fisher Information metric.
    Attributes
    ----------
    _a : int
        Size of matrix A.
    ...
    def __init__(self, a, b, n, k=1):
        self._a = a
        self._b = b
        self._n = n
        self._k = k
        weights = (b/(a*b), a/(a*b), 1/n)
        manifolds = (SpecialHermitianPositiveDefinite(a, k),
                     SpecialHermitianPositiveDefinite(b, k),
                     StrictlyPositiveVectors(n, k))
        super().__init__(manifolds, weights)
